...
impl Script {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let mut result =
            CompactSize::new(self.bytes.len() as u64).to_bytes();
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let Script =
            bytes[consumed..consumed + len].to_vec();

        Ok((Script::new(script), consumed + len))
    }
}
...
impl TransactionInput {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        // - Sequence (4 bytes)
        let (previous_output, mut offset) =
            OutPoint::from_bytes(bytes)?;

        let (script_sig, consumed) =
            Script::from_bytes(&bytes[offset..])?;
...
impl BitcoinTransaction {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        bytes.extend(
            CompactSize::new(self.inputs.len() as u64).to_bytes(),
        );
